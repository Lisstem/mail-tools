  ...
  module Util
    class Config
      ...
      def initialize(default: {}, files: [], env_prefix: nil, env_map: {}, env: ENV)
        ...
        files.reverse_each { |file| _merge(@store, object_apply(YAML.load_file(file), :to_s)) }
        _merge(@store, env_hash(env_prefix.to_s, object_apply(env_map, :to_s, values: true), env))
      end
      ...
      def self.create(name, files: :default, prompt: false, required: nil, **options)
